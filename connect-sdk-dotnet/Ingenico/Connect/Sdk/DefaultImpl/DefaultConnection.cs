...
namespace Ingenico.Connect.Sdk.DefaultImpl
{
    ...
    public class DefaultConnection : IPooledConnection
    {
            ...
            try
            {
                using (var message = new HttpRequestMessage(method, uri))
                {
                    ...
                    using (var httpResponse = await httpResponseTask)
                    {
                        ...
                        responseBody = LogResponse(guid, uri, httpResponse, httpResponse.Content.Headers, responseBody, duration);
        ...
        Stream LogResponse(Guid guid, Uri uri, HttpResponseMessage httpResponse, HttpContentHeaders responseBodyHeaders, Stream responseBodyStream, TimeSpan duration)
        {
            var logger = _communicatorLogger;
            if (logger == null)
            {

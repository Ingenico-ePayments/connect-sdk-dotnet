...
namespace Ingenico.Connect.Sdk.Merchant.Services
{
    ...
    public class ServicesClient : ApiResource
    {
        ...
        /// <summary>
        /// Resource /{merchantId}/services/settlementdetails/{paymentId}
        /// - <a href="https://epayments-api.developer-ingenico.com/s2sapi/v1/en_US/dotnet/services/settlementdetails.html">Get Settlement details</a>
        /// </summary>
        /// <param name="paymentId">string</param>
        /// <param name="context">CallContext</param>
        /// <returns>SettlementDetails</returns>
        /// <exception cref="ValidationException">if the request was not correct and couldn't be processed (HTTP status code BadRequest)</exception>
        /// <exception cref="AuthorizationException">if the request was not allowed (HTTP status code Forbidden)</exception>
        /// <exception cref="IdempotenceException">if an idempotent request caused a conflict (HTTP status code Conflict)</exception>
        /// <exception cref="ReferenceException">if an object was attempted to be referenced that doesn't exist or has been removed,
        ...
        ///            or the service that you're trying to reach is temporary unavailable (HTTP status code InternalServerError, BadGateway or ServiceUnavailable)</exception>
        /// <exception cref="ApiException">if the Ingenico ePayments platform returned any other error</exception>
        public async Task<SettlementDetails> Settlementdetails(string paymentId, CallContext context = null)
        {
            IDictionary<string, string> pathContext = new Dictionary<string, string>();
            pathContext.Add("paymentId", paymentId);
            string uri = InstantiateUri("/v1/{merchantId}/services/settlementdetails/{paymentId}", pathContext);
            try
            {
                return await _communicator.Get<SettlementDetails>(
                        uri,
                        ClientHeaders,
                        null,
                        context);
